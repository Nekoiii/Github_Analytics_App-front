...
require("dotenv").config(); //Required to access .env files// Use bodyParser to parse JSON
app.use(bodyParser.json()); //URL calls paths must be before paths below to default html page. Otherwise, it will not work
...
if (process.env.NODE_ENV != "production") {
  ...
  app.use(express.static(path.join(__dirname, "app"))); //Put this last among all routes. Otherwise, it will return index.html to all fetch requests and trip up CORS. They interrupt each other
  // For any request that doesn't match, this sends the index.html file from the app. This is used for all of our React code.
  app.get("*", (req, res) => {
  ...
  app.use(express.static(path.join(__dirname, "app/build"))); //Put this last among all routes. Otherwise, it will return HTML to all fetch requests and trip up CORS. They interrupt each other
  // For any request that doesn't match, this sends the index.html file from the app. This is used for all of our React code.
